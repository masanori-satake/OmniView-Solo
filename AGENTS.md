...
5. **ルートディレクトリの管理徹底**: 不具合調査や検証用の単体テストコード（`test*.html`, `test*.py` 等）、検証用画像、および個人の開発環境設定ファイル（`settings.json` 等）をリポジトリのルートディレクトリ等に誤ってコミット・放置しないこと。検証が必要な場合はローカル環境内のみで実施し、リポジトリに含めないこと。
