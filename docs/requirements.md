...
1. プロジェクト概要
本プロジェクトは、ハイブリッド会議（出社メンバーとリモートメンバーの混在）において、心理的距離を縮め、かつホワイトボードの視認性を劇的に向上させるためのChrome拡張機能（サイドパネル専用）である。
**外部のサーバーやクラウドAPIを一切利用せず、セキュリティが厳格な社内環境でもスタンドアロンで動作する**完全ローカル完結（Vanilla JS / 軽量画像処理）を徹底する。
...
- カメラ識別・自動設定復元機能: 同じハードウェア構成で繰り返し利用することを想定し、カメラの固有識別子を用いた自動復元機能を備える。
  - デバイスの識別と永続化: navigator.mediaDevices.enumerateDevices() で取得できる deviceId をキーとして、設定情報（役割、カスタムラベル）を chrome.storage.local に保存する。
  - 初期設定の自動適用: サイドパネル起動時、接続されたカメラのIDが保存済みデータと一致した場合、設定（役割・カスタムラベル）を初期値として自動的に適用・復元する。
- カメラ配置・レイアウト設定機能（M3対応レスポンシブ）
  - カメラ役割割り当て: M3スタイルのドロップダウン（Select）で、各カメラの役割を動的に変更可能とする。
...
- ホワイトボード補正・キャプチャ機能
  ...
  - 視認性向上（コントラスト加工）:
    - 適応的二値化（Adaptive Thresholding）またはCLAHE（ヒストグラム平坦化）を適用し、背景の白を飛ばしてマーカーの文字をくっきり強調する。
  - 写り込み（人・腕）の自動排除:
    - 数秒（例: 5秒）に1回の定周期でフレームをキャプチャし、過去複数枚のキャッシュ画像から時間的中央値（メディアン）フィルタを用いて合成画像を生成。動いている人間をノイズとして排除する。
  - 補正後画像のクリップボードコピー:
